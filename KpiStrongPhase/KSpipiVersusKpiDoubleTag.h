...
/**
 * KSpipiVersusKpiDoubleTag is a class for a BOSS algorithm
 * It runs over \f$D^0\bar{D^0}\f$ data and saves all events with a double \f$D\to K_S^0\pi^+\pi^-\f$ vs \f$D^0\to K\pi\f$ tag
...
#ifndef KSPIPIPIPIVERSUSKPIDOUBLETAG
#define KSPIPIPIPIVERSUSKPIDOUBLETAG
...
class KSpipiVersusKpiDoubleTag: public Algorithm {
  public: 
    /**
     * Default constructor for an algorithm where all necessary properties are declared
     */
    KSpipiVersusKpiDoubleTag(const std::string& name, ISvcLocator* pSvcLocator);
    /**
     * Trivial destructor
     */
    ~KSpipiVersusKpiDoubleTag();
    /**
